...
"""def addmoney_submission(request):
    if request.session.has_key('is_logged'):
        if request.method == "POST":
            ...
            add_money = request.POST["dropdownType"]
            quantity = request.POST["quantity"]
            Date = request.POST["Date"]
            category = None

            if add_money == 'Expense':
                # Get the selected category from the expense dropdown
                category = request.POST.get('Category1')
                print(category)
            elif add_money == 'Income':
                # Get the selected category from the income dropdown
                category = request.POST.get('Category2')

            add = Addmoney_info(user=user1, add_money=add_money, Date=Date, quantity=quantity, Category=category)
            add.save()
...
# Your existing views and imports...
    ...
    if request.session.has_key('is_logged'):
        if request.method == "POST":
            ...
            if add_money == 'Expense':
                category = request.POST.get('Category1')
                print(category)
                user_savings = user1.userprofile.Savings
                if user_savings < float(quantity):
                    send_expense_notification(user1, float(quantity), user_savings)
...
def send_expense_notification(request):
    print(request.user)
    subject = 'Expense Tracker Notification'
    message = render_to_string('home/expense_notification_email.html', {
        """"'user': request.user,
        'expense_amount': 90,
        'saved_money': 80,"""
...
# Attempt to get the value of "add_money" from request.POST
# add_money = request.POST["add_money"]
# except MultiValueDictKeyError:
# Handle the case when "add_money" is not present in the form data
# add_money = None
